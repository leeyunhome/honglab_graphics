// example8.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	const string filename("example8.ppm");
	...
	for (int row = 0; row < image_height; ++row)
	{
		for (int col = 0; col < image_width; ++col)
		{
			...
			if (col >= 20 && col <= 70 && row >= 30 && row <=80)
			{
				buffer[offset] = 255;
				buffer[offset + 1] = 242;
				buffer[offset + 2] = 0;
